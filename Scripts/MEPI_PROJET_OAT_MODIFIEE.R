#####  OAT STANDARD POUR FONCTION MODIFIEE  -------------------------------------------
...
dimnames(oat) <- list(c(1:(length(ValNominale)*10)), c("tx_morbidite", "incidence_t730", "pic_infectieux", "prevalence_annee_1"))
...
as_mean["processus"] <- c(rep("demo", 10), rep("epidemio", 13))
as_se["processus"] <- c(rep("demo", 10), rep("epidemio", 13))
...
for(i in levels(oat_gg$sortie)){
  print(
    ggplot(data = oat_gg[oat_gg$sortie == i,], mapping = aes(x = index, y = as)) +
      facet_wrap(~params, ncol = 5, nrow = 5, scales = "free_y",  
