...
modAppli <- function(parametre){  
  ...
  # boucle des scénarios de l'échantillonnage de l'AS (car on peut faire entrer plusieurs jeux de parametres pour que le modele tourne plusieurs fois)
  for (i in 1:nrow(parametre)) { 
    ...
    for (t in 1:(temps-1)) { 
      ...
      # classe d'âge xx (la deuxieme ligne)
      ...
      # classe d'âge xx (lq troisieme ligne)
      ...
      # calcul des effectifs par état de santé (la quatrieme ligne donc)
...
ValNominale = c(100, 0.5, 0.0014,
                0.00029, 0.0019, 0.0019, 0.0082, 5, 1/365, 1/365, 0.3, 1/5, 1/20,
                1/100, 0.001)
